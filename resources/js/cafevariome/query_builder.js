...
$(function() {
    ...
    function load_phen_json() {
        ...
        .done((jsonData)=> { 
            ...
            $.each( jsonData[0].alt,function( key, value ){
                $('select.values_altall').append($('<option></option>').attr('value', value.toLowerCase()).text(value))
            });

            $.each(jsonData[0].ref,function( key, value ) {
                $('select.values_refall').append($('<option></option>').attr('value', value).text(value))
            });
            ...
            // })
            i = 0;
            $.each(phen_data.transcript, function(key, value){
                $('select.hgvs').append($('<option></option>').attr('value', value + ':' + phen_data.cDNA[i]).text(value + ':' + phen_data.cDNA[i]));
                i++;
            });
            i = 0;
            $.each(phen_data.transcript, function(key, value){
                $('select.hgvs').append($('<option></option>').attr('value', value + ':' + phen_data.protein[i]).text(value + ':' + phen_data.protein[i]));
                i++;
            });
    ...
    $('#build_query').click(() => {
        ...
        .done((jsonAPI) => {
            ...
            //     logic_gender = []
            //     eav.push({'attribute' : "sex", 'operator': "is", 'value': "m"})
            //     logic_gender.push("/query/components/eav/" + (eav.length-1))
            //     eav.push({'attribute' : "sex", 'operator': "is", 'value': "f"})
            //     logic_gender.push("/query/components/eav/" + (eav.length-1))
            //     eav.push({'attribute' : "sex", 'operator': "is", 'value': "u"})
            //     logic_gender.push("/query/components/eav/" + (eav.length-1))

            //     logic['-AND'].push({'-OR': logic_gender})
            // }
            if ($('#genmale').prop('checked')) {
                logic_eav(['sex', 'is', 'm'], eav, logic);
                logic['-AND'].push("/query/components/eav/" + (eav.length-1))    
            }
            else if($('#genfemale').prop('checked')){
                logic_eav(['sex', 'is', 'f'], eav, logic);
                logic['-AND'].push("/query/components/eav/" + (eav.length-1))    
            }

            var min_age = $("#age-range").slider("values", 0);
            var max_age = $("#age-range").slider("values", 1);

            var abs_min_age = $("#age-range").slider("option", 'min');
            var abs_max_age = $("#age-range").slider("option", 'max');

            var thisYear = new Date().getFullYear();
            var monthStr = '-01'; 
            if (min_age > abs_min_age) {
                logic_eav(['date_of_birth', 'dt<=', (thisYear - min_age).toString() + monthStr], eav, logic);
            }
            if (max_age < abs_max_age) {
                logic_eav(['date_of_birth', 'dt>=', (thisYear - max_age).toString() + monthStr], eav, logic);
            }

            var min_diag_age = $("#age-diagnosis-range").slider("values", 0);
            var max_diag_age = $("#age-diagnosis-range").slider("values", 1);

            var abs_min_diag_age = $("#age-diagnosis-range").slider("option", 'min');
            var abs_max_diag_age = $("#age-diagnosis-range").slider("option", 'max');

            if (min_diag_age > abs_min_diag_age) {
                logic_eav(['age_at_diagnosis', '>=', min_diag_age], eav, logic);
            }
            if (max_diag_age < abs_max_diag_age) {
                logic_eav(['age_at_diagnosis', '<=', max_diag_age], eav, logic);
            }

            var min_symptom_age = $("#age-first-symptoms-range").slider("values", 0);
            var max_symptom_age = $("#age-first-symptoms-range").slider("values", 1);

            var abs_min_symptom_age = $("#age-first-symptoms-range").slider("option", 'min');
            var abs_max_symptom_age = $("#age-first-symptoms-range").slider("option", 'max');

            if (min_symptom_age > abs_min_symptom_age) {
                logic_eav(['age_at_onset', '>=', min_symptom_age], eav, logic);
            }
            if (max_symptom_age < abs_max_symptom_age) {
                logic_eav(['age_at_onset', '<=', max_symptom_age], eav, logic);
            }
            ...
            $('#gen_container .rule').each(function() {
                v = {
                    ...
                    'position' : $('input.values_start', this).val(),
                    'ref' : $('select.values_refall', this).val(),
                    'alt' : $('select.values_altall', this).val()
                };
